...
evidence are ready. operator infrastructure's config, ingress declaration and exact plugin pin
...
is deterministic application evidence. On 2026-09-08 the owner confirmed that the
...
the owner is reconsidering whether a separate Agent Connect gateway is needed at all.
...
Requested by the owner on 2026-09-06. History rewriting is acceptable if needed, but
