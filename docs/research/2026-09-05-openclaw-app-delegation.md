...
The product requirement clarified by the owner is: an app lends capabilities to the
...
Mapping every app to the owner's profile does not separate those apps from one another.
...
by the owner. Provider OAuth and MCP-client OAuth are different directions of trust.
...
the owner clarified that verified website origins are not a universal requirement.
