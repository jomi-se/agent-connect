...
The compact hero diagram is a dependency-free port of the supplied `Micro flow` concept. A ten-second loop distinguishes the outbound task, agent-to-app tool calls, and app-to-agent results while the three actor nodes react in sequence. The bracket around the gateway and coding agent marks the user-owned runtime boundary. Animation pauses offscreen and becomes a legible static diagram under reduced motion. It is an explanatory overview, not a substitute for the later event-driven live visualization.
